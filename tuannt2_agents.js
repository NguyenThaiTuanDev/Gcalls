...
const apiUrl = 'https://api.gcalls.co/api/v1/readAllAgents'; // Endpoint là Agents
...
async function fetchDataAndImport(pageLeft, startPage) {
  ...
  while (hasMore && startPage <= end_page) {
    ...
    for (const record of allResults) {
      ...
      const createResponse = await fetch(`${nocodbUrl}/api/v2/tables/mnf2yvyvmu4teh6/records`, options);
