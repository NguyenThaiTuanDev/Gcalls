...
const apiUrl = 'https://api.gcalls.co/api/v1/readManyActivities'; // Endpoint là Activities
...
async function fetchDataAndImport(pageLeft, startPage) {
  ...
  while (hasMore && startPage <= end_page) {
    ...
    for (const record of allResults) {
      ...
      const createResponse = await fetch(`${nocodbUrl}/api/v2/tables/mdsn1oz0ycoi8il/records`, options);
