...
const apiUrl = 'https://api.gcalls.co/api/v1/readManyContacts'; // Endpoint là Contacts
...
async function fetchDataAndImport(pageLeft, startPage) {
  ...
  while (hasMore && startPage <= end_page) {
    ...
    for (const record of allResults) {
      ...
      const createResponse = await fetch(`${nocodbUrl}/api/v2/tables/m4cf0d0p1qmp5ko/records`, options);
